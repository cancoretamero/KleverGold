 ...
 *  - Muestra: fecha actual, último precio, Δ diaria y CAGR (desde 1944-07-22).
 *  - Mini sparkline 60 días con tooltip “liquid glass”.
 *
 * Props
 *  - rows: Array<{ date: Date, close: number, ... }>
 *  - onAppendRows?: (rowsNew: Array<Row>) => void
 *  - fetchMissingDaysSequential?: (daysISO: string[]) => Promise<Array<Row>>
 */
...
export default function GoldNowSection({
  ...
  const cagr = useMemo(() => {
    ...
    return Math.pow(end / Math.max(start, 1e-9), 1/years) - 1;
  }, [ordered]);
